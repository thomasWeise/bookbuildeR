...
.relativize.path <- function(relPath, sourceDir, rootDir) {
  if(is.null(relPath) || (length(relPath) != 1L)) {
    exit("Error in \\relpath{", relPath, "} in file '", sourceFile, "'.");
  }
  return(path.relativize(check.file(file.path(sourceDir, relPath[1L])), rootDir));
}
...
.preprocess.input <- function(sourceFile, rootDir, output) {
  ...
  for(line in lines) {
    ...
    line <- preprocess.regexp(regex="\\\\relative.path\\{(.+)\\}",
                              func=.relativize.path,
                              text=line,
                              sourceDir=sourceDir,
                              rootDir=rootDir);
