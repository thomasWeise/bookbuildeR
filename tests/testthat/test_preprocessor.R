...
test_that("Test preprocess.doc no bibliography", {
  dir <- tempfile();
  ...
  dest <- preprocess.doc(root, "vv.md", bibliography=FALSE);
...
test_that("Test preprocess.doc bibliography", {
  dir <- tempfile();
  dir.create(dir, showWarnings = FALSE, recursive = TRUE);
  dir <- normalizePath(dir);
  ...
  dest <- preprocess.doc(root, "vv.md", bibliography=TRUE);
  expect_identical(dest, file.path(dir, "vv.md"));
  con <- file(dest, "rt");
  lines <- readLines(con);
  close(con);
